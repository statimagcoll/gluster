...
Convergence failed for the CD8 marker. Looking at the histogram and parameter table, it looks like that marker had a small number of nonzero cells. Adjusting the constraints might improve the fit. We add in a lower bound of 1/2 for the expressed cell population for CD8. Then we refit the data with new constraints for all the markers based on visualization of previous histograms.
...
constrCfGMMbunch = groupGluster(cell[,nzNormedMarkers], slide=cell$slide_id, boundaryMarkers=boundaries2, qboundaryMarkers = quantileBoundaries2, n.cores = 12, ncores=1)
...
readRDS('/media/disk2/atlas_mxif/data/cfGMM_cody_fit.rds')
marker = 7
...
quantileBoundaries2 = list(matrix(c(0,.8, .9, 1), byrow = TRUE, nrow=2),
                          ...
                          NULL,
                          NULL)
boundaries2 = list(matrix(c(-Inf,.3, .5, Inf), byrow = TRUE, nrow=2),
                  matrix(c(-Inf,.3, .5, Inf), byrow = TRUE, nrow=2),
                  matrix(c(-Inf,.3, .6, Inf), byrow = TRUE, nrow=2),
                  matrix(c(-Inf, 0, .5, 1.5), byrow = TRUE, nrow=2),
                  matrix(c(-Inf,.3, .45, .6), byrow = TRUE, nrow=2),
