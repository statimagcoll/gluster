...
### Adjusting the boundary constraints {.tabset}
...
```{r}
constrCfGMMbunch = readRDS('/media/disk2/atlas_mxif/data/cfGMM_cody_fit2.rds')
marker = 2
for(marker in names(boundaries)){
  cat('\n####', marker, '\n')
  plot(constrCfGMMbunch, marker=marker)
...
merged = unsplit(lapply(names(constrCfGMMbunch), function(x){
  ...
  names(constrCfGMMbunch[[x]]$expressionZ) = paste0('cfGMMPhenoMarg_', names(constrCfGMMbunch[[x]]$expressionZ))
  rownames(constrCfGMMbunch[[x]]$expressionZ) = paste0(x, rownames(constrCfGMMbunch[[x]]$expressionZ))
  constrCfGMMbunch[[x]]$expressionZ}), cell$slide_id)
cell[,names(merged)] = merged


#debug(kappaGroupGluster)
# 
test = kappaGroupGluster(posterior = cell[, paste0('cfGMMPheno_nzNorm_Median_Cell_', phenotypes)], marginal=cell[, paste0('cfGMMPhenoMarg_nzNorm_Median_Cell_', phenotypes)], standard = cell[,phenotypes], batch = cell$slide_id)
